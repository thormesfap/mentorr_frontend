...
function Star({rating, is_small, mouseClick, updateable}: {
  rating: number;
  is_small: boolean;
  updateable: boolean;
  ...
  const displayRating = updateable && hoverRating > 0 ? hoverRating : Math.floor(rating);

  return (
    <figure className="flex"
      onMouseLeave={() => updateable && setHoverRating(0)}
    >
      {[...Array(5)].map((_, index) => (
        <svg
          onMouseOver={() => 
        ...
        >
          <path
            ...
            fill={index < displayRating ? "#F5BF14" : ""}
            stroke={index >= displayRating ? "#F5BF14" : "none"}
          />
        </svg>
      ))}
    </figure>
